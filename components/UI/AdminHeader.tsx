...
export default function AdminHeader({ onMenuToggle, sidebarOpen }: AdminHeaderProps) {
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    if (searchQuery.trim()) {
      ...
      // You can redirect to search results or filter current page
    }
  }
    ...
    <header className="bg-white/80 backdrop-blur-md border-b border-gray-200 sticky top-0 z-40">
      <div className="flex justify-between items-center px-3 sm:px-4 lg:px-6 py-3 sm:py-4">
        ...
        <div className="flex items-center space-x-2 sm:space-x-4">
          ...
          <div className="flex items-center space-x-2 sm:space-x-3">
            ...
            <div className="hidden sm:block">
              <h1 className="text-sm lg:text-lg font-semibold text-gray-900 leading-tight">
                Admin Dashboard
              </h1>
        ...
        <div className="flex items-center space-x-1 sm:space-x-2 lg:space-x-4">
          ...
          <button 
            onClick={handleLogout}
            className="hidden sm:block lg:hidden p-2 text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-colors"
            aria-label="Logout"
          >
            <LogOut className="w-4 h-4" />
          </button>
      ...
      {/* Mobile Search Bar */}
      {showMobileSearch && (
        <div className="md:hidden px-3 sm:px-4 pb-3 sm:pb-4 border-t border-gray-200 bg-white/95">
          <form onSubmit={handleSearch} className="relative">
            ...
            <input
              ...
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-10 pr-10 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white text-sm"
              autoFocus
            />
            <button
              type="button"
